...
struct ConvolutionalLayer {
	...
	array<image<kernel_width, in_channels>, out_channels> W { };
	...
	auto forward (const image<N, in_channels>& X_unpadded) {
		...
		auto Y { convolve(pad<N, in_channels, padding_width>(X_unpadded), W) };
...
#pragma GCC ivdep
	...
	auto backward (const image<N, in_channels>& X, const image<M, out_channels>& grad_Y) {
		...
		for (int g = 0; g < in_channels; g++) {
			auto X_g { pad<N, padding_width>(X[g]) };
		...
		auto grad_Y_padded { pad<M, out_channels, kernel_width - 1>(grad_Y) };
