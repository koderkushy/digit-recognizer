...
struct MaxPool {
	...
	auto recurse (const image<_N, channels>& _X, const int label) {
		static constexpr int N = _N + 2 * P;
		static constexpr int M = (N - K + S) / S;

		auto X { pad<_N, channels, P>(_X) };
	...
	template<uint64_t _N, uint64_t channels>
	auto evaluate (const image<_N, channels>& _X, const int label) {
	...
	auto backward (const image<N, channels>& X, const image<M, channels>& grad_Y) {
		...
		return pad<N, channels, -P>(grad_X);
	}
